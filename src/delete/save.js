...
const App = () => {
  ...
  const handleFormSubmit = (event) => {
    ...
    addLocalStorageItem('data-notes', JSON.stringify(textareaResult));
  }
  ...
  const editNote = (event, key) => {
    editLocalStorageItem(key, 'textarea', event.target.value);
  }
  ...
  // filtred search
  const searchFilter = getLocalStorage?.filter(({ textarea }) => { 
    return textarea.toLowerCase().includes(search);
  });

  return (
    <div className='app bg-white dark:bg-gray-800 ease-out duration-300'>
              ...
              return (
                <div className='h-[200px]' key={key}>
                  <form className='relative h-full grid'>
                    <textarea className='w-full resize-none h-40 outline-none p-4 text-black bg-green-400' value={item.textarea} onChange={(event) => editNote(event, key)} required />
                    <div className='grid grid-cols-3 items-center justify-between h-[40px] bg-green-300'>
                      <span className='w-auto h-full leading-[40px] pr-4 pl-4'></span>
                      <button type='submit' className='w-auto h-full pr-4 pl-4' onClick={saveEditNote}>Save Edit</button>
                    </div>  
                  </form>
                </div>
              )
